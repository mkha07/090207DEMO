...
from utility import RES_DIR, ensure_pixmap, ROLE_ADMIN, ROLE_MANAGER, ROLE_GUEST, STAFF_ROLES
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, db: Database, user_data):
        ...
        self.current_view = VIEW_PRODUCTS
    ...
    @property
    def role(self):
        return self.user_data.get("role_name", ROLE_GUEST)
    ...
    def setup_interface(self):
        ...
        username = self.user_data.get("user_name", ROLE_GUEST)
        self.username_label.setText(f"{username} ({self.role})")

        self.products_btn.setVisible(self.role in STAFF_ROLES)
        self.orders_btn.setVisible(self.role in STAFF_ROLES)
        self.filter_widget.setVisible(self.role in STAFF_ROLES)
        self.control_panel.setVisible(self.role == ROLE_ADMIN)

        logo = os.path.join(RES_DIR, "Icon.png")
        self.logo_label.setPixmap(ensure_pixmap(logo, QSize(100, 100)))
    ...
    def setup_combos(self):
        ...
        for item in self.db.get_suppliers():
    ...
    def load_grid(self):
        ...
        if self.current_view == VIEW_ORDERS:
        ...
        if not data:
            self.card_grid.addWidget(QLabel("Отсутствует информация"), 0, 0)
            return

        on_edit = self.edit_card if self.role == ROLE_ADMIN else None
        on_select = self.select_card if self.role == ROLE_ADMIN else None
        for i, row in enumerate(data):
            card = card_class(row, self.role, on_edit, on_select)
    ...
    def _check_no_open_dialog(self):
        if self.dialog_opened:
            QMessageBox.critical(self, "Ошибка", "Нельзя открывать несколько окон добавления/редактирования")
            return False
        return True

    def edit_card(self, data):
        if not self._check_admin() or not self._check_no_open_dialog():
            return
        if self.current_view == VIEW_ORDERS:
    ...
    def delete(self):
        ...
        if self.selected_card is None:
            ...
            return
        if self.current_view == VIEW_ORDERS:
            order_id = self.selected_card.data.get("order_id")
            msg = QMessageBox(self)
            msg.setWindowTitle("Внимание!")
            msg.setText("Вы уверены, что хотите удалить данный заказ?")
            ...
            product_id = self.selected_card.data.get("product_id")

...
class Database:
    ...
    def _execute(self, sql, args=None):
        """Выполняет INSERT/UPDATE/DELETE, делает commit и возвращает lastrowid."""
        with self.conn.cursor() as cursor:
            cursor.execute(sql, args)
    ...
    def authenticate(self, login, password):
        user_data = self._query("SELECT u.user_id, u.user_name, r.role_name "
                            "FROM users u JOIN roles r ON u.role_id = r.role_id WHERE user_login = %s"
                            " AND user_pass = %s", (login, password))
        return user_data

    def get_suppliers(self):
        return self._query("SELECT * FROM suppliers")

    def get_types(self):
        return self._query("SELECT * FROM product_types")

    def get_categories(self):
        return self._query("SELECT * FROM categories")

    def get_manufacturers(self):
        return self._query("SELECT * FROM manufacturers")
    ...
    def is_product_in_order(self, product_id):
        rows = self._query("SELECT count(*) as cnt FROM order_items where product_id = %s", product_id)
        count = rows[0].get('cnt', 0)
        return count > 0

    def del_product(self, product_id):
        return self._execute("DELETE FROM products WHERE product_id = %s", product_id)

    def get_orders(self):
        return self._query("""SELECT o.*, s.status_name, d.delivery_address FROM
                            orders o 
                            JOIN order_statuses s ON o.status_id = s.status_id
                            JOIN delivery_adresses d ON o.delivery_id = d.delivery_id;""")

    def get_clients(self):
        return self._query('SELECT u.user_id, u.user_name, r.role_name '
                       'FROM users u JOIN roles r ON u.role_id = r.role_id')

    def get_statuses(self):
        return self._query("SELECT * FROM order_statuses")

    def insert_new_address(self, delivery_address):
        delivery_id = None
        rows = self._query("SELECT * FROM delivery_adresses WHERE lower(delivery_address) = %s",
                       str(delivery_address).lower())
        if rows:
            delivery_id = rows[0].get("delivery_id")

        return delivery_id if delivery_id else self._execute(
